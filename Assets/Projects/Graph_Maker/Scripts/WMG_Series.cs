...
public class WMG_Series : MonoBehaviour {
	...
	void OnEnable () {
		//描画しているオブジェクトが非アクティブからアクティブになった際に続きから描画を再開する
		if (graduallySetPointFlow != null) {
			//折れ線グラフの描画が途中で終わっていれば再開する
			StartCoroutine (graduallySetPointFlow);
		}
	}
	...
	/// ポイントを追加し、グラフに反映します
	/// </summary>
	/// <param name="pointVals">Point vals.</param>
	public void AddPointValue(Vector2 pointVal) {
		//_pointValuesを変更することによってインスペクタの値が変更されます
		//_pointValuesを変更するだけではグラフの見た目に反映されないため、グラフの見た目変更時に使用されるpointValuesに値をセットし、
		//ポイントの値が変更された事をイベントを発行することで知らせます
		_pointValues.Add(pointVal);
		pointValues.SetList (_pointValues);
	}
	IEnumerator graduallySetPointFlow;
	/// <summary>
	/// ポイントを設定し、グラフに反映します
	/// </summary>
	/// <param name="pointVals">Point vals.</param>
	public void SetPointValues(List<Vector2> pointVals) {
		pointVals = OptimizationPointValue (pointVals, 5f);	//最適化
		graduallySetPointFlow = SetPointValuesGradually (pointVals);
		if (this.gameObject.activeInHierarchy) {
			//オブジェクトが非アクティブの時にコルーチンを動作させるとエラーになるため回避
			StartCoroutine (graduallySetPointFlow);
		}
	}

	/// <summary>
	/// グラフ描画負荷軽減のためにグラフに設定するポイントを最適化する(不要な頂点を減らす)
	/// </summary>
	/// <returns>The point value.</returns>
	/// <param name="originalValue">最適化を適用するグラフのポイントリスト(X,Y)</param>
	/// <param name="optimizeAngle">
	/// 3つの頂点を選んで、繋いだ時の角度でまっすぐとみなす値(0~180f)
	/// 最適化の影響度合い大きいほど強くかかる</param>
	List<Vector2> OptimizationPointValue (List<Vector2> originalValue, float optimizeAngle) {
		float optimizeIntensity01 = Mathf.Clamp01 (optimizeAngle / 180f);
		//確認用
		Debug.Log ("OptimizationStart-------------------------------");
		Debug.Log ("Intensity:" + optimizeIntensity01);
		Debug.Log ("BeforePointCount:" + originalValue.Count);
		//頂点数が3つ以上ない場合は処理が行えないためリターン
		if (originalValue.Count < 3)
			return originalValue;
		List<Vector2> optimizedPointValue = new List<Vector2> ();
		//始点を追加
		optimizedPointValue.Add (originalValue [0]);
		//はじめから3つの頂点を取得していく
		int lastDeleteCount = 0;	//真ん中の頂点を削除したとき、次の処理ではじめの頂点の位置を削除した頂点にならないように補正するためのカウント
		for (int i = 0; i < originalValue.Count - 2; i++) {
			var point1 = originalValue [i - lastDeleteCount];	//はじめの頂点(3つの内)
			var point2 = originalValue [i + 1];					//真ん中の頂点(3つの内)
			var point3 = originalValue [i + 2];					//最後の頂点(3つの内)
			var vec1to2 = (point2 - point1).normalized;	//1→2の方向ベクトル
			var vec2to3 = (point3 - point2).normalized;	//2→3の方向ベクトル
			//折れ線の曲がり具合を見るために、1→2ベクトルと2→3ベクトルの内積を計算
			float curveFactor = Vector2.Dot (vec1to2, vec2to3);
			//今のままでは、1→2と2→3の角度が0度→90度→180度で、内積が1→0→-1となり曲がり具合の係数として扱い辛いため以下のように変換する
			//(まっすぐ:0)～(直角:0.5f)～(限りなく鋭角：1f)
			curveFactor *= -1f;
			curveFactor += 1f;
			curveFactor *= 0.5f;
			curveFactor = Mathf.Clamp01 (curveFactor);	//念のため0~1fの範囲に収める
			//曲がり具合が、ある一定の値より小さければ、まっすぐな線とみなして不要な真ん中の頂点を消す
			if (curveFactor < Mathf.Clamp01 (optimizeIntensity01)) {
				//不要な頂点なら追加しない
				lastDeleteCount++;
			} else {
				//必要な頂点なら追加する(真ん中の頂点)
				optimizedPointValue.Add (point2);
				lastDeleteCount = 0;
			}
		}
		//終点を追加
		optimizedPointValue.Add (originalValue [originalValue.Count - 1]);
		//確認用
		Debug.Log ("AfterPointCount:" + optimizedPointValue.Count);
		Debug.Log ("OptimizeRate:" + ((float)optimizedPointValue.Count / (float)originalValue.Count) * 100f + "%");
		Debug.Log ("OptimizationEnd---------------------------------");
		return optimizedPointValue;
	}
	...
	public void ClearPointValues() {
		if (graduallySetPointFlow != null) {
			//コルーチンが実行途中であれば
			StopCoroutine (graduallySetPointFlow);	//実行途中のコルーチンをストップ
			graduallySetPointFlow = null;
		}
		_pointValues.Clear ();
		pointValues.Clear ();
	}
	...
	/// <summary>
	/// ポイントの値の変化を購読するメソッドを追加します
	/// </summary>
	/// <param name="method">Method.</param>
	public void AddPointValueObserver(Action<List<Vector2>> method) {
		pointValuesValC.OnChange += () => {
			method(_pointValues);
		};
	}
	...
	[SerializeField] private bool _isUseOriginalXValue;		//ポイントに設定したX軸の位置を変換せず利用するか

	/// <summary>
	/// ポイントに設定したX軸の位置を変換せず利用するか
	/// falseの場合、設定したXの値にかかわらずポイントは一定の間隔で配置される
	/// </summary>
	/// <value><c>true</c> if is use original X value; otherwise, <c>false</c>.</value>
	public bool isUseOriginalXValue {
		get {
			return _isUseOriginalXValue;
		}
		set {
			_isUseOriginalXValue = value;
		}
	}
	...
	void getNewPointPositionsAndSizes(List<GameObject> prevPoints) {
		...
		for (int i = 0; i < points.Count; i++) {
			...
			if (isUseOriginalXValue) {
				float posRate = pointValues[i].x / theGraph.xAxis.AxisMaxValue;
				float width = theGraph.xAxisLength;
				newX = posRate * width;
			} else {
	...
	void updateAreaShading() {
		...
		if (areaShadingUsesComputeShader && areaShadingRects.Count == 1) { // all values (min, max, and the points) represent a percentage of graph
			...
			for (int i = 0; i < pointValues.Count; i++) {
				...
				//Debug
				if (areaShadingCSdata.pointVals.Length <= (4 * i + 1)) {
					Debug.Log ("Error:pointVals out of range" + ",pointVal.Length = " + areaShadingCSdata.pointVals.Length + ",index = " + i);
					break;
				}
				areaShadingCSdata.pointVals[4 * i] = pointPos.x;	//pointValsの要素数が不足
			...
			try {
				...
				Debug.Log ("computeShade.SetFloats:" + areaShadingCSdata.pointVals.Length);
			}
