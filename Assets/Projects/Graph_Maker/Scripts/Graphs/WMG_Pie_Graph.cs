...
public class WMG_Pie_Graph : WMG_Graph_Manager {
	...
	/// スライスを追加し、グラフに反映します
	/// </summary>
	/// <param name="sliceVal">Slice value.</param>
	public void AddSlice (float val,Color color,System.String label = null) {
		_sliceValues.Add (val);
		sliceValues.SetList (_sliceValues);
		_sliceColors.Add (color);
		sliceColors.SetList (_sliceColors);
		if (label != null) {
			_sliceLabels.Add (label);
	...
	/// スライスを設定し、グラフに反映します
	/// </summary>
	/// <param name="sliceVals">Slice vals.</param>
	/// <param name="sliceColors">Slice colors.</param>
	/// <param name="sliceLabels">Slice labels.</param>
	public void SetSlices (List<float> vals,List<Color> colors,List<System.String> labels = null) {
		if (!(vals.Count == colors.Count && ((labels == null) || (labels.Count == vals.Count))))
			return;		//要素数が違う場合は無視する
		_sliceValues = vals;
		sliceValues.SetList (_sliceValues);
		_sliceColors = colors;
		sliceColors.SetList (_sliceColors);
