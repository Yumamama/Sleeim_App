	...
	where T : class, new()
{
	...
	public CSVReader (string filePath, int skipLineCount)
		: this (filePath, skipLineCount, null)
	{
	}

	public CSVReader (string filePath, int skipLineCount, Encoding encoding)
	{
		/*	Unity5.4の仕様変更により不要になった
		// 拡張子の確認
		...
		//this.reader = new StringReader (this.filePath);
		var stream = new FileStream(this.filePath, FileMode.Open, FileAccess.Read, FileShare.ReadWrite); //他プロセスがファイルアクセス中でも読込を行う
